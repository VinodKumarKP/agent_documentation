...
```bash
pip install oai-openai-agent-core
```
...
pip install "oai-openai-agent-core[vector-required]"

# For ChromaDB support
pip install "oai-openai-agent-core[chromadb]"

# For Postgres (pgvector) support
pip install "oai-openai-agent-core[postgres]"

# For S3 vector store support
pip install "oai-openai-agent-core[s3]"

# For all features
pip install "oai-openai-agent-core[all]"
```
...
When you use the `oai-gen` tool to create a new OpenAI agent project, it generates a standardized, production-ready directory structure. This ensures consistency and makes it easy to locate and manage different parts of your agent.
...
-   **`agents/my_agent/agent.py`**: This is where the core `OpenAIAgent` class is instantiated. You typically don't need to modify this file unless you are customizing the agent's fundamental behavior.
